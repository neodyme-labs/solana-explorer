...
export const fromProgramData = (programData: ProgramDataAccountInfo): SecurityTXT | undefined => {
    ...
    return content.reduce<number[][]>((prev, current) => {
        if (current === 0) {
            prev.push([]);
        } else {
        ...
        return prev
    }, [[]]).map(c => String.fromCharCode(...c)).reduce<{ map: {}, key: string | undefined }>((prev, current) => {
        const key = prev.key;
        if (!key) {
            return {
                map: prev.map,
    ...
    }, { map: {}, key: undefined }).map as SecurityTXT;
}
